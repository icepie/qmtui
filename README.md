...
标准 `--web` 模式下，CLI 是播放状态与音频输出的唯一来源；浏览器中的账号、曲库（歌单／专辑／歌手收藏）、搜索（歌曲／歌手／歌单／专辑）、最近播放、播放与暂停、播放模式、队列（下一首播放、加入队列）、进度跳转、收藏、评论、音质、下载与设置都会通过本地 API 与 SSE 同步到同一个 CLI 会话。歌曲行的 ⋯ 与右键复用客户端自带的菜单（播放、查看评论、复制信息、添加到歌单、下载、分享，自建歌单另有删除）。若 GStreamer 不可用，程序会回退到浏览器音频输出。
