...
// UNUSED EXPORTS: Storage, onStoreInit, onStoreUpdate
...
;// CONCATENATED MODULE: ./node_modules/stook/dist/stook.esm.js
...
var emitter = /*#__PURE__*/mitt_es();
var STORE_INITED = 'STORE_INITED';
var STORE_UPDATED = 'STORE_UPDATED';
function emitStoreInit(key) {
  emitter.emit(STORE_INITED, key);
}
function emitStoreUpdate(data) {
  emitter.emit(STORE_UPDATED, data);
}
function onStoreInit(cb) {
  emitter.on(STORE_INITED, function (key) {
    cb(key);
  });
}
function onStoreUpdate(cb) {
  emitter.on(STORE_UPDATED, function (data) {
    cb(data);
  });
}
...
var Store = /*#__PURE__*/function () {
  function Store(value) {
    ...
    this.setState = function (key, value) {
      ...
      _this.setters.forEach(function (set) {
  ...
  _proto.getNextState = function getNextState(value) {
    ...
    if (typeof value !== 'function') return value; // can not use immer

    if (typeof this.state !== 'object') return value(this.state);
    var useImmer = true;
    var immerState = fn(this.state, function (draft) {
      var fnValue = value(draft); // use function return value
...
function useStore(key, initialValue) {
  ...
  var initialState = storageStore ? storageStore.state : initialValue;
...
//# sourceURL=webpack://qqmusic/./node_modules/stook/dist/stook.esm.js_+_2_modules?